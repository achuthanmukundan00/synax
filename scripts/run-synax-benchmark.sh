...
#   SYNAX_CMD                  Path to Synax CLI (default: bun dist/cli.js from repo root)
...
#   test-output.txt             Test run output (bun run test)
...
if [ -z "$SYNAX_CMD" ]; then
  SYNAX_CMD="bun $REPO_ROOT/dist/cli.js"
fi
