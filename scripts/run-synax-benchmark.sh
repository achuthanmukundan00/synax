...
ARTIFACTS_DIR="$ARTIFACTS_PARENT/$RUN_ID"
...
# Initialize git in the workdir so Synax can detect changes
cd "$WORKDIR"
git init --quiet
...
# Generate .synax.toml for the workdir using bench env vars
cat > "$WORKDIR/.synax.toml" <<SYNAXCONF
# Auto-generated by synax auto-research benchmark harness
[provider]
kind = "openai-compatible"
base_url = "${SYNAX_BENCH_BASE_URL:-http://localhost:8080/v1}"
model = "${SYNAX_BENCH_MODEL:-local}"
api_key = "${SYNAX_BENCH_API_KEY:-not-needed}"
...
cd "$WORKDIR"
node test/validate-email.test.js > "$ARTIFACTS_DIR/test-output.txt" 2>&1 || true
TEST_EXIT_CODE=$?
