 ...
 *
 * Usage:
 *   node scripts/score-synax-benchmark.mjs <artifacts-dir>
 *
...
function main() {
  ...
  if (!artifactsDir) {
    console.error('Usage: node score-synax-benchmark.mjs <artifacts-dir>');
    process.exit(1);
  }
...
/**
 * Check if Synax changed any source files (not just .synax.toml, package-lock, etc.)
 */
...
function isSourceFile(path) {
  const nonSource = ['.synax.toml', 'package-lock.json', 'node_modules', '.git'];
