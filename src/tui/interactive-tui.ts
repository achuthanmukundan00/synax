...
export async function runInteractiveTui(
  ...
  options?: {
    ...
    /** Effective settings shown by the interactive settings screen. */
  ...
  const loadSettingsConfig = (): EffectiveSynaxConfig => {
    if (options?.settingsConfig) return options.settingsConfig;
    try {
...
function isLocalEndpoint(endpoint: string): boolean {
  return /(?:^|\/\/)(?:127\.0\.0\.1|localhost|0\.0\.0\.0)(?::|\/|$)/i.test(endpoint);
}
